...
declare namespace ProfileService {
    ...
    /**
     * Set to true to use a mock in-memory DataStore instead of Roblox DataStore. Useful for testing and local development. Note that this mock DataStore does not persist data between server sessions, and is not shared between servers. Also, this mock DataStore does not simulate all edge cases of Roblox DataStore, so be cautious when using it for testing.
     */
    let UseMockDataStore: boolean;
    /**
     ...
     *	})
     */
    const IssueSignal: RBXScriptSignal<
     ...
     */
    const CorruptionSignal: RBXScriptSignal<
        (profileStoreName: string, profileKey: string) => void
    >;
     ...
     */
    const CriticalStateSignal: RBXScriptSignal<
        (isCriticalState: boolean) => void
    >;
    ...
    const GetProfileStore: <
        ProfileTemplate extends object,
        RobloxMetaData extends unknown = unknown
    >(
        profileStoreIndex:
            | string
            | {
